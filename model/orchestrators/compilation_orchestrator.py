...
# ─────────────────────────────────────────────
# STEP 3 – Teams notification for compilation
# ─────────────────────────────────────────────

# Teams webhook — override via settings.json "notifications.teams_webhook_url"
# or environment variable BENTO_TEAMS_WEBHOOK
TEAMS_WEBHOOK_URL = ""
try:
    ...
    if os.path.exists(_settings_path):
        ...
        TEAMS_WEBHOOK_URL = (
            _settings.get("notifications", {}).get("teams_webhook_url", "")
        )
except Exception:
    pass


def send_compile_teams_notification(
    jira_key:    str,
    hostname:    str,
    env:         str,
    status:      str,
    ...
    """
    Send Teams notification on TP compilation completion.
    Non-fatal — compilation succeeded even if notification fails.

    Args:
        jira_key    : JIRA issue key (e.g. TSESSD-1234)
        hostname    : Tester hostname (e.g. IBIR-0383)
        env         : Environment (e.g. ABIT)
        status      : "success" | "failed" | "timeout"
        detail      : Human-readable detail string
        elapsed     : Elapsed time in seconds
        tgz_file    : Name of the compiled TGZ file (if success)
        webhook_url : Override webhook URL (optional)
        logger      : Logger instance (optional)
        log_callback: GUI log callback (optional)
    ...
    if not url:
        _log(logger,
             "⚠ Teams webhook URL not configured — skipping compile notification.",
    ...
    try:
        ...
        icon    = "✅" if status.lower() == "success" else (
                  "⏱" if status.lower() == "timeout" else "❌")
        color   = ("0076D7" if status.lower() == "success"
                   else "FFA500" if status.lower() == "timeout"
                   else "D40000")

        facts = [
            {"name": "Status",   "value": status.upper()},
            {"name": "Tester",   "value": f"{hostname} ({env})"},
            {"name": "Detail",   "value": detail},
            {"name": "Elapsed",
             "value": f"{elapsed}s ({elapsed // 60}m {elapsed % 60}s)"},
            {"name": "Time",
             "value": datetime.now().strftime("%Y-%m-%d %H:%M:%S")},
        ]
        if tgz_file:
            facts.insert(2, {"name": "TGZ File", "value": tgz_file})

        payload = {
            "@type":      "MessageCard",
            "@context":   "http://schema.org/extensions",
            "themeColor": color,
            "summary":    f"BENTO TP Compile {status.upper()} — {jira_key}",
            "sections": [{
                "activityTitle":    f"{icon} BENTO TP Compilation — "
                                    f"{status.upper()}",
                "activitySubtitle": f"Tester: {hostname} ({env})  |  "
                                    f"JIRA: {jira_key}",
                "facts": facts,
            }]
        }
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            if resp.status == 200:
                _log(logger, "✓ Teams compile notification sent.", log_callback)
                return True
            else:
                _log(logger,
        ...
        # Non-fatal — compilation succeeded, notification is best-effort
        _log(logger, f"⚠ Teams compile notification failed (non-fatal): {e}",
             log_callback, "warning")
        return False
