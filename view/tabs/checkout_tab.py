...
class CheckoutTab(BaseTab):
    ...
    def _build_result_collection_section(self, parent, row):
        ...
        cfg1.columnconfigure(6, weight=1)
        ...
        ttk.Button(cfg1, text="…", width=3,
                   command=self._rc_browse_mids).grid(row=0, column=2, padx=(0, 2))
        self._rc_gen_mids_btn = ttk.Button(
            cfg1, text="📝 Generate", width=10,
            command=self._rc_generate_mids_file)
        self._rc_gen_mids_btn.grid(row=0, column=3, padx=(0, 12))
        _tip(self._rc_gen_mids_btn,
             "Auto-generate MIDs.txt from the Profile Table above.\n"
             "Each row with a MID + Primitive + Dut becomes a line:\n"
             "  MID  P{Primitive}D{Dut}  ProfileXmlName  False\n\n"
             "The file is saved next to the CHECKOUT_QUEUE folder.")

        ttk.Label(cfg1, text="Target Folder:").grid(row=0, column=4, sticky=tk.W, padx=(0, 4))
        self._rc_target_var = tk.StringVar()
        ttk.Entry(cfg1, textvariable=self._rc_target_var, width=35).grid(
            row=0, column=5, sticky="we", padx=(0, 4))
        ttk.Button(cfg1, text="…", width=3,
                   command=self._rc_browse_target).grid(row=0, column=6)
    ...
    def _rc_generate_mids_file(self):
        """Auto-generate MIDs.txt from the Profile Table data.

        Each profile row with a MID, Primitive, and Dut produces a line:
            MID  P{Primitive}D{Dut}  {ProfileXmlName}  False

        The file is saved via a Save-As dialog so the user can choose
        the location.  After saving, the MIDs File entry is auto-populated.
        """
        if not self._profile_data:
            self.show_error(
                "No Profile Data",
                "The Profile Table is empty.\n\n"
                "Please add MID rows to the Profile Table first\n"
                "(either manually or by importing from CRT Excel / XML).",
            )
            return
        ...
        lines = []
        skipped = []
        for i, row in enumerate(self._profile_data):
            mid       = str(row.get("MID", "")).strip()
            primitive = str(row.get("Primitive", "")).strip()
            dut       = str(row.get("Dut", "")).strip()
            material  = str(row.get("Material_Desc", "")).strip()

            if not mid:
                skipped.append(f"Row {i + 1}: missing MID")
                continue
            if not primitive or not dut:
                skipped.append(f"Row {i + 1} (MID={mid}): missing Primitive/Dut")
                continue

            # Build location in P{Primitive}D{Dut} format
            location = f"P{primitive}D{dut}"

            # Build a descriptive name (Material_Desc or MID itself)
            name = material if material else mid

            lines.append(f"{mid}  {location}  {name}  False")

        if not lines:
            detail = "\n".join(skipped) if skipped else "No rows with MID + Primitive + Dut found."
            self.show_error(
                "Cannot Generate MIDs.txt",
                f"No valid MID entries found in the Profile Table.\n\n{detail}",
            )
            return

        # Show skipped rows as warning if any
        if skipped:
            self.log(f"⚠ MIDs.txt generation: skipped {len(skipped)} row(s):")
            for s in skipped:
                self.log(f"   {s}")
        ...
        jira_key = self.context.get_var('issue_var').get().strip().upper()
        header = (
            f"# MIDs.txt — Auto-generated by BENTO\n"
            f"# JIRA: {jira_key or 'N/A'}\n"
            f"# Generated: {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            f"# Format: MID  LOCATION  FILE_NAME  SPOOL(True/False)\n"
            f"# Lines starting with $ are marked as collected.\n"
            f"#\n"
        )
        content = header + "\n".join(lines) + "\n"

        # Default filename
        default_name = f"MIDs_{jira_key}.txt" if jira_key else "MIDs.txt"
        ...
        try:
            ...
            self.show_error("Save Error", f"Failed to save MIDs.txt:\n{e}")
            return

        # Auto-populate the MIDs File entry
        self._rc_mids_file_var.set(save_path)
        self.log(f"✓ MIDs.txt generated: {save_path}  ({len(lines)} MID(s))")
        messagebox.showinfo(
            "MIDs.txt Generated",
            f"Successfully generated MIDs.txt with {len(lines)} MID(s).\n\n"
            f"File: {save_path}\n\n"
            f"The MIDs File path has been auto-populated.",
